        ...
        Else
            ...
            Try
                ...
                If division = "ALL" Then
                    ...
                    xlsWorkBook.Close()
                Else
                    Dim query As String = "SELECT DISTINCT targetdata.period, targetdata.dept, targetdata.employeeid, targetdata.name,targetdata.status, targetdata.title, targetdata.detail, targetdata.difficulty, targetdata.weight, targetdata.scomment1,Sachievement1, result.comment1, result.achievement1, title2, detail2, difficulty2, weight2, scomment2,Sachievement2, result.comment2, result.achievement2, title3, detail3, difficulty3, weight3, scomment3,Sachievement3, result.comment3, result.achievement3, result.difficulty, result.achievement,result.evaluation, Jir1, Jir2, Jiritsu, jin1, jin2, Jinsoku, ren1, ren2, Renkei, osc, ose, mbo, competency, total, notable, devplan, employee.app1, manager.name, employee.app2 FROM targetdata INNER JOIN employee ON targetdata.employeeid = employee.EmployeeID INNER JOIN result ON targetdata.employeeid = result.employeeid INNER JOIN manager ON manager.employeeid = employee.app1 WHERE employee.Division ='" & division & "' AND targetdata.period ='" & period & "' AND result.period ='" & period & "' ORDER BY employee.dept;"
                    Dim dscmd As New MySqlDataAdapter(query, conn)
                    Dim ds As New DataSet
                    dscmd.Fill(ds)
                    xlsWorkBook = xlsApp.Workbooks.Open("C:\PAPVN\Export_format.xlsx")
