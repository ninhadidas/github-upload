...
Public Class SBMS_BusSelect
    ...
    'Sub BusyGridBus_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    '    Dim SDA As New MySqlDataAdapter
    '    Dim dbDataSet As New DataTable
    '    Dim count As Integer
    '    conn = New MySqlConnection With {
    ...
    '    Try
    '        conn.Open()
    '        Dim query As String = "SELECT bus_name, tbl_bus_quota.quota FROM tbl_businfo INNER JOIN tbl_order ON tbl_order.bus_id = tbl_businfo.bus_id INNER JOIN tbl_bus_quota ON tbl_order.bus_id = tbl_bus_quota.bus_id WHERE tbl_order.end_time <= now();"
    '        command = New MySqlCommand(query, conn)
    '        reader = command.ExecuteReader
    '        count = 0
    '        While reader.Read
